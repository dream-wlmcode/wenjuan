...
$(function() {
    ...
    /***************************************获取问卷信息 s************************************/
    var id = $("#e_qSnaireModal #e_id",window.parent.document).val();
    $.ajax({
            ...
            if (data.code == 200) {
                ...
                if(data.data.questions.length > 0){
                    ...
                    for(var i=0;i<data.data.questions.length;i++){
                          ...
                          if(type != "3"){
                              html +='<label onselectstart="return false;" class="ismust" for="msck_'+parseInt(timu_listlen+1)+'">(是否必选<input type="checkbox" class="msck" id="msck_'+parseInt(timu_listlen+1)+'" '+cked+' />)</label>';
                          }else{
                              html +='<label onselectstart="return false;" class="ismust" for="msck_'+parseInt(timu_listlen+1)+'">(是否必填<input type="checkbox" class="msck" id="msck_'+parseInt(timu_listlen+1)+'" '+cked+' />)</label>';
                          }
                          ...
                          var options = data.data.questions[i].options;
    ...
    var editqSnaire_beginDate1 = $("#editqSnaire_beginDate1").datetimepicker({ //选择年月日
        //显示中文
        language: 'zh-CN',
        //显示格式
    ...
    var editqSnaire_endDate1 = $("#editqSnaire_endDate1").datetimepicker({ //选择年月日
        //显示中文
        language: 'zh-CN',
        //显示格式
        format: 'yyyy-mm-dd hh:ii',
    ...
    editqSnaire_beginDate1.on("changeDate",
    function(e) {
        $('#editqSnaire_endDate1').datetimepicker('setStartDate', e.date);
    });
    editqSnaire_endDate1.on("changeDate",
    function(e) {
        $('#editqSnaire_beginDate1').datetimepicker('setEndDate', e.date);
    });
    ...
    //下拉问卷分类列表
    $("#editqSnaire_categoryName").click(function(event) {
    ...
    $("#editqSnaire_categoryName").siblings('.downtb').click(function(event) {
        event.stopPropagation();
        if($("#addqSnaire-selectCot").is(":hidden")){
          $("#addqSnaire-selectCot").show();
    ...
    function(e, row, element) {
        $("#editqSnaire_categoryName").attr("value", row.id);
        $("#editqSnaire_categoryName").val(row.id);
        $("#editqSnaire_categoryId").attr("value", row.id);
        $("#editqSnaire_categoryId").val(row.id);
        $("#addqSnaire-selectCot").hide();
        $("#addqSnaire-selectCot").animate({marginTop:"50px",opacity:"0"},"fast");
    });
    ...
    $("#e_qSubjectModal_save").click(function(event) {
        ...
        // var timu_listlen = $("#timu_List").find(".timu_box").length;
        var html = '';
        html +='<div class="timu_box timu_box'+tmLen+'" data-id="'+id+'" data-type="'+type+'" data-qbid="undefined" ><div class="timu_title">';
...
function loadingRightqS() {
    ...
    $.initBootstrapTable("#selectqS-table", {
        ...
        onUncheck:function(row, $element){ //取消选中
            var id = row.id;
            cked_Array.splice($.inArray(id, cked_Array), 1);
            $("#timu_List").find(".timu_box[data-qbid='"+id+"']").remove();
            $("#timu_List").find(".sxnum").each(function(index, el) {
                $(this).html(parseInt(index+1));
            });
            ...
            $("#timu_List").find(".timu_box").each(function(index, el){
                $(this).attr("data-id",parseInt(index+1));
...
function addLefttimu_list(data){
        var timuxx_len = 1;
        var timu_listlen = $("#timu_List").find(".timu_box").length;
        var id = $("#timu_List").find(".timu_box"+timu_listlen+"").attr("data-id");
        var qbId = data.data.id;
        var categoryId = data.data.categoryId;
...
function timuEdit(e,title,type,options,status,categoryId){
    ...
    var id = $(e).parents(".timu_box"+tmLen+"").attr("data-id");
...
function timuDetele(e){
    ...
    $("#timu_List").find(".timu_box").each(function(index, el){
        // var id = $("#timu_List").find(".timu_box"+timu_listlen+"").attr("data-id");
        // $(this).attr("data-id",parseInt(parseInt(id)-timu_listlen+index));
        $(this).attr("data-id",parseInt(index+1));
        $(this).attr("class","timu_box timu_box"+parseInt(index+1)+"");
        $(this).find(".tmEditBtn").attr("data-listlen",""+parseInt(index+1)+"");
    });
